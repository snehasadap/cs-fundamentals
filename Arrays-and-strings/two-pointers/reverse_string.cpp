...
public:
    void reverseString(vector<char>& s) {
        ...
        while (i < j){
           ...
           s[i] = s[j]; //since we have temp, we can safely assign the right value to the left. now, the left is equal to the right value
           s[j] = temp; //we take the temp which has the original value of the left and assign it to the right value to "swap" the indices   
            i++; //once that is complete, we increment and decrement the left and right pointers, respectively, to repeat the process for the rest of the chars
            j--;
        }
